...
class TestBulletBowlStacking(unittest.TestCase):
  ...
  planner_config = {'random_orientation': True, 'dpos': 0.02, 'drot': np.pi/16}

  def testPlanner2(self):
    self.env_config['render'] = True
    self.env_config['seed'] = 0
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'pybullet', 'close_loop_household_picking', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
