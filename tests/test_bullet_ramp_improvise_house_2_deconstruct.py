...
class TestBulletRampHouseBuilding2Deconstruct(unittest.TestCase):
  ...
  env_config = {'workspace': workspace, 'max_steps': 10, 'obs_size': 90, 'render': False, 'fast_mode': True,
                'seed': 0, 'action_sequence': 'xyzrrrp', 'num_objects': 3, 'random_orientation': True,
  ...
  def testPlanner(self):
    ...
    num_processes = 20
    env = env_factory.createEnvs(num_processes, 'pybullet', 'ramp_improvise_house_building_2_deconstruct', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
