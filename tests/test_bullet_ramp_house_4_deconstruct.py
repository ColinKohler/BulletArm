...
class TestBulletRampHouseBuilding4Deconstruct(unittest.TestCase):
  ...
  def testPlanner(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'pybullet', 'ramp_house_building_4_deconstruct', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
