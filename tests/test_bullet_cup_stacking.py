...
class TestBulletCupStacking(unittest.TestCase):
  workspace = np.asarray([[0.3, 0.67],
                          [-0.2, 0.2],
                          [0, 0.50]])
  env_config = {'workspace': workspace, 'max_steps': 10, 'obs_size': 128, 'render': False, 'fast_mode': True,
                'seed': 0, 'action_sequence': 'pxyzr', 'num_objects': 4, 'random_orientation': False,
  ...
  def testPlanner2(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'pybullet', 'cup_stacking', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
