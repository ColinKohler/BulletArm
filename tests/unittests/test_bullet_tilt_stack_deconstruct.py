...
class TestBulletBlockStackingDeconstruct(unittest.TestCase):
  workspace = np.asarray([[0.35, 0.65],
                          [-0.15, 0.15],
                          [0, 0.50]])
  env_config = {'workspace': workspace, 'max_steps': 10, 'obs_size': 90, 'render': False, 'fast_mode': True,
                'seed': 0, 'action_sequence': 'pxyzrr', 'num_objects': 4, 'random_orientation': True,
  ...
  def testPlanner(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'rl', 'pybullet', 'tilt_block_stacking_deconstruct', self.env_config, {})
    total = 0
    s = 0
    step_times = []
    env.reset()
