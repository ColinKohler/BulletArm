...
class TestBulletHouse1Deconstruct(unittest.TestCase):
  ...
  env_config = {'workspace': workspace, 'max_steps': 10, 'obs_size': 90, 'render': False, 'fast_mode': True,
                'seed': 0, 'action_sequence': 'pxyrr', 'num_objects': 3, 'random_orientation': True,
  ...
  def testPlanner(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'rl', 'pybullet', 'tilt_improvise_house_building_2', self.env_config, {})
    while True:
      env.reset()
      pass
