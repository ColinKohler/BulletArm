...
class TestBulletBowlStacking(unittest.TestCase):
  workspace = np.asarray([[0.3, 0.7],
                          [-0.2, 0.2],
                          [0.01, 0.25]])
  env_config = {'workspace': workspace, 'max_steps': 100, 'obs_size': 80, 'render': False, 'fast_mode': True,
                'seed': 0, 'action_sequence': 'pxyzr', 'num_objects': 2, 'random_orientation': True,
  ...
  def testPlanner2(self):
    ...
    num_processes = 1
    env = env_factory.createEnvs(num_processes, 'pybullet', 'close_loop_house_building_1', self.env_config, self.planner_config)
    total = 0
    s = 0
    step_times = []
    env.reset()
