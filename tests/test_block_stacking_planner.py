...
for sim in range(100):
  _, hand_obs, obs = envs.reset()
  done = False
  s = 0
  while not done:
    s+=1
    action = envs.getNextAction()
    obs_, reward, done = envs.step(action)
    _, hand_obs_, obs_ = obs_
    x_pix = (action[2] - workspace[1,0]) / heightmap_resolution
    y_pix = (action[1] - workspace[0,0]) / heightmap_resolution

    if plot:
      fig, ax = plt.subplots(nrows=1, ncols=4)
      ax[0].imshow(obs, cmap='gray', vmin=0.0)
      ax[0].scatter([x_pix], [y_pix], s=1, c='r')
      ax[1].imshow(obs_, cmap='gray', vmin=0.0)
      ax[1].scatter([x_pix], [y_pix], s=1, c='r')
      ax[2].imshow(hand_obs, cmap='gray', vmin=0.0)
      ax[3].imshow(hand_obs_, cmap='gray', vmin=0.0)
      plt.show()

    if done and reward and not envs.didBlockFall():
      print('{}: s'.format(sim))
      success += 1
    if done and not reward and not envs.didBlockFall():
      print('{}: f'.format(sim))
      other += 1
    if envs.didBlockFall():
      print('{}: f'.format(sim))
      falls += 1
      done = True
