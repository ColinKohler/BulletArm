...
class DrawerHandle:
  def __init__(self, drawer_id, fixed=True):
    ...
    if self.grip_type == BOX:
      grip_visual_id = pb.createVisualShape(
        pb.GEOM_BOX,
        halfExtents=[self.grip_width/2, self.grip_length/2, self.grip_width/2],
        rgbaColor=[1, 1, 1, 1]
      )
      grip_collision_id = pb.createCollisionShape(
        pb.GEOM_BOX,
        halfExtents=[self.grip_width/2, self.grip_length/2, self.grip_width/2]
      )
      ...
      grip_visual_id = pb.createVisualShape(
        ...
        radius=self.grip_width/2,
        rgbaColor=[1, 1, 1, 1],
        visualFrameOrientation=pb.getQuaternionFromEuler([np.pi/2, 0, 0])
      )
      grip_collision_id = pb.createCollisionShape(
        pb.GEOM_CYLINDER,
        height=self.grip_length,
        radius=self.grip_width/2,
        collisionFrameOrientation=pb.getQuaternionFromEuler([np.pi/2, 0, 0])
      )
    else:
      raise NotImplementedError

    if self.sidebar_type == BOX:
      left_visual_id = pb.createVisualShape(
        pb.GEOM_BOX,
        halfExtents=[self.sidebar_length / 2, self.sidebar_width / 2, self.sidebar_width / 2],
        rgbaColor=[1, 1, 1, 1]
      )
      left_collision_id = pb.createCollisionShape(
        pb.GEOM_BOX,
        halfExtents=[self.sidebar_length / 2 - 0.01, self.sidebar_width / 2 - 0.01, self.sidebar_width / 2 - 0.01]
      )

      right_visual_id = pb.createVisualShape(
        pb.GEOM_BOX,
        halfExtents=[self.sidebar_length / 2, self.sidebar_width / 2, self.sidebar_width / 2],
        rgbaColor=[1, 1, 1, 1]
      )
      right_collision_id = pb.createCollisionShape(
        pb.GEOM_BOX,
        halfExtents=[self.sidebar_length / 2 - 0.01, self.sidebar_width / 2 - 0.01, self.sidebar_width / 2 - 0.01]
      )
      ...
      left_visual_id = pb.createVisualShape(
        pb.GEOM_CYLINDER,
        length=self.sidebar_length,
        radius=self.sidebar_width/2,
        rgbaColor=[1, 1, 1, 1],
        visualFrameOrientation=pb.getQuaternionFromEuler([0, np.pi/2, 0])
      )
      left_collision_id = pb.createCollisionShape(
        pb.GEOM_CYLINDER,
        height=self.sidebar_length - 0.01, # Have to make these not touch drawer front otherwise drawer is stuck closed
        radius=self.sidebar_width/2,
        collisionFrameOrientation=pb.getQuaternionFromEuler([0, np.pi/2, 0])
      )
    ...
    self.id = pb.createMultiBody(
      0.1, grip_collision_id, grip_visual_id,
      basePosition = grip_position,
      baseOrientation = pb.getQuaternionFromEuler([0, 0, 0]),
      linkMasses=[.1, .1],
      linkCollisionShapeIndices=[left_collision_id, right_collision_id],
