...
class CloseLoopPegInsertionPlanner(CloseLoopPlanner):
  ...
  def setNewTarget(self):
    ...
    pre_insert_pos[2] += hole_z_offset + 5e-4
    ...
    if self.stage == 0:
      self.hole_x_offset = npr.uniform(-0.01, 0.01)
      self.hole_y_offset = npr.uniform(-0.01, 0.01)
      ...
      self.stage = 3
      self.current_target = (pre_insert_pos, pre_insert_rot, constants.PICK_PRIMATIVE, 2e-3, 1e-1)
    elif self.stage == 3:
      # insert peg
      ...
      self.stage = 0
      self.current_target = (insert_pos, insert_rot, constants.PICK_PRIMATIVE, 2e-3, 1e-1)
