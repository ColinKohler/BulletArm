...
class CloseLoopPegInsertionPlanner(CloseLoopPlanner):
  ...
  def setNewTarget(self):
    ...
    if self.stage == 0:
      self.hole_x_offset = npr.uniform(-0.10, 0.10)
      self.hole_y_offset = npr.uniform(-0.10, 0.10)
      ...
      self.stage = 2
      self.current_target = (drag_pos_2, drag_rot_2, constants.PICK_PRIMATIVE, 1e-3, None)
    elif self.stage == 2:
      # Drag peg to hole
      ...
      self.stage = 0
      self.current_target = (insert_pos, insert_rot, constants.PICK_PRIMATIVE, 1e-3, 1e-1)
